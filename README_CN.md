...
摘要: 对ML模型预测的反事实(CF)解释为受预测结果不利影响的个人提供了可操作的追索权建议。然而，尽管最终用户喜欢使用CF解释，但事实证明，CF解释在现实世界的应用程序中会带来重大的安全风险；尤其是，恶意攻击者可以利用CF解释对底层专有ML模型执行查询高效的模型提取攻击。为了解决这一安全挑战，我们提出了一种新的模型无关的水印框架CFMark，用于检测基于CF解释的未经授权的模型提取攻击。CFMark涉及一个新的双层优化问题，该问题将不可区分的水印嵌入到生成的CF解释中，以便使用零假设显著性检验(NHST)方案来检测使用这些水印的CF解释的任何未来的模型提取攻击。同时，嵌入的水印不会影响CF解释的质量。我们在不同的真实世界数据集、CF解释方法和模型提取技术中评估CFMark。我们的实验结果证明了CFMark的有效性，在使用带水印的CF解释识别未经授权的模型提取攻击时，F-1得分为~0.89。重要的是，该水印只会导致生成的CF解释的质量几乎可以忽略不计的下降(即有效性下降约1.3%，邻近度下降约1.6%)。我们的工作为在现实世界应用程序中安全部署CF解释奠定了关键基础。
...
摘要: 本文介绍了一种新的无数据模型提取攻击，该攻击在效率、准确性和有效性方面显著提高了当前的技术水平。传统的黑匣子方法依赖于使用受害者的模型作为先知来标记高置信度区域内的大量样本。这种方法不仅需要大量的查询，还会导致模型的准确性和可转移性较差。相比之下，我们的方法的创新之处在于，专注于对低置信度区域(沿着决策边界)进行采样，并使用进化算法来优化采样过程。这些新的贡献使攻击者所需的查询数量大幅减少了10倍到600倍，同时提高了被盗模型的准确性。此外，我们的方法改进了边界对齐，使得从被盗模型到受害者模型的对抗性实例具有更好的可转移性(将攻击成功率从平均60%提高到82%)。最后，我们在不了解目标的体系结构或数据集的情况下，通过对受害者进行严格的黑箱假设来实现所有这些。我们在三个分辨率越来越高的数据集上演示了我们的攻击，并将我们的性能与四种最先进的模型提取攻击进行了比较。



## **39. PEAS: A Strategy for Crafting Transferable Adversarial Examples**

PEAS：制作可转移对抗示例的策略 cs.LG
...
摘要: 黑盒攻击对机器学习系统构成了重大威胁，对手对目标模型的了解有限。用替代模型生成的对抗性例子通常会受到转移到目标模型的限制。虽然最近的工作探索了为了提高成功率而进行的排名扰动，但这些方法只看到了有限的收益。我们提出了一种称为PEAS的新策略，该策略可以提高现有黑盒攻击的可转移性。豌豆利用了这样一种洞察力，即感知上等价的样本在其对抗性转移能力方面表现出显著的变异性。我们的方法首先通过微妙的增强从初始样本生成一组图像。然后，我们使用一组替代模型来评估这些图像上的对抗性扰动的可转移性。最后，选取可转移性最强的对抗性实例进行攻击。我们的实验表明，PEAS可以将现有攻击的性能提高一倍，攻击成功率平均比现有排名方法提高2.5倍。我们在ImageNet和CIFAR-10上对豌豆进行了全面的评估，分析了超参数的影响，并提供了一项消融研究，以确定每个组件的重要性。
...
超越修剪标准：微调和自适应比率在神经网络稳健性中的主导作用 cs.LG
...
摘要: 深度神经网络(DNN)在图像识别和自然语言处理等任务中表现出色，但其日益复杂的程度使其在资源受限环境中的部署变得更加复杂，并增加了对对手攻击的敏感性。虽然传统的剪枝方法减小了模型的大小，但它们往往会损害网络抵御微妙扰动的能力。本文挑战了传统的强调权重重要性评分作为剪枝网络性能的主要决定因素的观点。通过广泛的分析，包括在CIFAR、Tiny-ImageNet和各种网络体系结构上进行的实验，我们证明了有效的微调在提高性能和对抗健壮性方面发挥着主导作用，往往超过了所选择的剪枝标准的影响。为了解决这个问题，我们引入了模块稳健敏感度，这是一种新的度量，它根据每个网络层对对手干扰的敏感度自适应地调整剪枝比率。通过将该度量集成到剪枝过程中，我们开发了一种稳定的算法，同时保持了准确性和健壮性。实验结果表明，我们的方法能够实际部署更健壮和更高效的神经网络。



## **44. Toward Robust RALMs: Revealing the Impact of Imperfect Retrieval on Retrieval-Augmented Language Models**

迈向稳健的RALM：揭示不完美检索对检索增强语言模型的影响 cs.CL
...
摘要: 检索增强语言模型(RALM)因其能够生成准确答案和提高效率而受到广泛关注。然而，由于对不完美的检索者或知识源的依赖，RALM天生就容易受到不完美信息的影响。我们确定了三种常见的场景--无法回答、对抗性、冲突--在这些场景中，检索到的文档集可能会将RALM与可信的真实世界示例混淆。我们提出了第一个全面的调查，以评估RALM检测和处理此类问题场景的情况如何。在这些场景中，我们提出了一种新的对抗攻击方法--基于产生式模型的对抗攻击(GenADV)和一种新的附加文档下的度量稳健性(RAD)，以系统地检验对抗稳健性。我们的发现表明，RALMS经常无法识别文档集的不可回答或矛盾，这经常导致幻觉。此外，我们还表明，添加一个对手会显著降低RALM的性能，当两个场景重叠(对抗性+不可回答)时，该模型变得更加脆弱。我们的研究确定了评估和增强RALMS稳健性的关键领域，为开发更健壮的模型奠定了基础。
...
摘要: 对抗性训练（AT）指的是将对抗性示例（通过可显着影响模型预测的不可察觉的扰动而改变的输入）集成到训练过程中。最近的研究证明了AT在提高深度神经网络抵御各种对抗攻击的鲁棒性方面的有效性。然而，仍然缺乏对这些事态发展的全面概述。这项调查通过回顾广泛的最近和代表性研究来解决这一差距。具体来说，我们首先描述AT的实施过程和实际应用，然后从数据增强、网络设计和训练配置三个角度对AT技术进行全面回顾。最后，我们讨论了AT中的共同挑战，并为未来的研究提出了几个有前途的方向。
...
摘要: 视觉语言模型(VLM)(如CLIP)的健壮性对于它们在自动驾驶、医疗诊断和安全系统等安全关键型应用中的部署至关重要，在这些应用中，对视觉和文本数据的准确解释是必不可少的。然而，这些模型非常容易受到对手攻击，这可能会严重损害它们在现实世界场景中的性能和可靠性。以前的方法主要集中在通过对抗性训练和使用FGSM、AutoAttack和DeepFool等模型生成对抗性示例来提高健壮性。然而，这些方法通常依赖于强假设，例如固定的扰动规范或预定义的攻击模式，并且涉及到很高的计算复杂性，使得它们在实际环境中的实施具有挑战性。本文结合多种攻击策略和先进的机器学习技术，提出了一种新的对抗性训练框架，以显著增强VLMS对多种对抗性攻击的健壮性。在CIFAR-10和CIFAR-100等真实数据集上进行的实验表明，该方法显著增强了模型的稳健性。微调的剪辑模型在相反的扰动图像上的准确率达到了43.5%，而基线模型的准确率只有4%。神经网络模型在这些具有挑战性的分类任务中达到了98%的高准确率，而XGBoost模型在预测任务中的准确率达到了85.26%。



## **48. Class-RAG: Content Moderation with Retrieval Augmented Generation**

Class-RAG：具有检索增强生成的内容审核 cs.AI
...
摘要: 健壮的内容审核分类器对于生成式人工智能系统的安全至关重要。内容审核或安全分类是出了名的模棱两可：安全和不安全输入之间的差异通常非常微妙，这使得分类器(甚至是人类)很难在没有进一步背景或解释的情况下正确区分违规和良性样本。此外，随着这些技术在不同的应用程序和受众中部署，通过持续的模型微调来扩展风险发现和缓解变得越来越具有挑战性和成本。为了应对这些挑战，我们提出了一种使用检索扩展生成的分类方法(Class-RAG)。Class-RAG通过访问检索库来扩展其基本LLM的能力，检索库可以动态更新，以实现语义热修复，从而立即、灵活地降低风险。与传统的微调模型相比，Class-RAG在决策过程中表现出灵活性和透明度。实证研究表明，Class-RAG在分类上优于传统的分类算法，并且对对手攻击具有更强的鲁棒性。此外，我们的研究结果表明，Class-RAG的性能随检索库的大小而变化，这表明增加库的大小是改善内容审核的一种可行且低成本的方法。
...
注意差距：基础模型和军事情报、监视和瞄准的秘密扩散 cs.CY
...
摘要: 关于基础模型的双重用途及其构成的风险的讨论压倒性地集中在一小部分用例和国家安全指令上--特别是人工智能如何能够有效地建设一类被称为CBRN的系统：化学、生物、放射和核武器。对这些假设和狭隘主题的压倒性关注，阻碍了关于当前人工智能在军事系统中的应用，特别是ISTAR：情报、监视、目标获取和侦察的亟需的对话。这些都是人工智能实际部署中最根深蒂固的使用，这些部署对平民构成生死攸关的风险，在这种情况下，滥用和失败会造成地缘政治后果和军事升级。商业模型的广泛可获得性所带来的新的扩散风险，以及缺乏可靠地防止它们对ISTAR能力作出贡献的有效办法，尤其突显了这一点。在这篇文章中，我们概述了当前和预期在CBRN背景之外使用商业基础模型所产生的重大国家安全问题，并批评了CBRN焦点(例如计算阈值、模型权重释放)导致的政策辩论的缩小。我们证明，在商业基础模型中，无法阻止个人身份信息对ISTAR能力做出贡献，可能会导致对手使用和扩散军事人工智能技术。我们还展示了在军事环境中使用基础模型如何内在地扩展了军事系统及其所连接的国防基础设施的攻击矢量。我们的结论是，为了确保军事系统的安全，限制人工智能武器的扩散，可能有必要将军事人工智能系统和个人数据与商业基础模型隔离。
